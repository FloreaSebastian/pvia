...
BEGIN
  IF TG_OP = 'DELETE' THEN
    UPDATE solar_models s SET layout_version = s.layout_version + 1,
           layout_hash = solar_layout_fingerprint(s.company_id, s.id)
...
UPDATE public.solar_models s SET layout_hash = public.solar_layout_fingerprint(s.company_id, s.id)
 WHERE s.layout_hash IS NULL;
...
DECLARE _geo integer; _lv integer; _lh text; _design_id uuid; _s jsonb; _sid uuid; _mid text; _pos integer;
...
BEGIN
  ...
  SELECT geometry_version, layout_version, layout_hash INTO _geo, _lv, _lh FROM solar_models
  ...
  IF _geo <> _expected_geometry_version OR _lv <> _expected_layout_version
     OR _lh IS DISTINCT FROM _expected_layout_hash
