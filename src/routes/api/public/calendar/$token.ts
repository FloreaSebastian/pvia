    ...
    handlers: {
      GET: async ({ params }) => {
        ...
        const { data: t } = await supabaseAdmin
          ...
          .select("id,company_id,scope,revoked_at")
        ...
        let q = supabaseAdmin
          .from("pv")
          .select("id,numero,type,status,reception_date,signed_at,description,client_id,chantier_id,created_at")
...
function renderIcs(rows: Row[], calName: string): string {
  ...
  for (const r of rows) {
    ...
    if (r.signed_at) {
      ...
      // all-day, no DTEND needed but include for compat
    } else {
