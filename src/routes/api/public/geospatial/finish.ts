    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const { data, error } = await supabaseAdmin.rpc("solar_job_finish", {
          ...
          _error_code: body.error_code ?? undefined,
          _error_message: body.error_message ?? undefined,
          _result_version: body.result_version ?? undefined,
          _pipeline_version: body.pipeline_version ?? undefined,
          _engine_version: body.engine_version ?? undefined,
        ...
        if (body.status === "COMPLETED" && body.result) {
          ...
          const { error: insertError } = await supabaseAdmin.from("solar_engine_results").insert({
            ...
            pipeline_version: body.pipeline_version ?? undefined,
            engine_version: body.engine_version ?? undefined,
            source_provider: sources[0]?.provider ?? undefined,
            source_dataset: sources[0]?.dataset ?? undefined,
            source_crs: origin.source_crs ?? undefined,
            working_crs: origin.working_crs ?? undefined,
