          ...
          <ul className="divide-y text-sm">
            {d.pastDue.map((s: any, i: number) => (
              <li key={i} className="flex justify-between py-1.5">
                <Link to="/admin/companies/$id" params={{ id: s.company_id }} className="hover:underline font-mono text-xs">{s.company_id.slice(0, 8)}…</Link>
          ...
          <ul className="divide-y text-sm">
            {d.emailFailures.map((e: any) => (
              <li key={e.id} className="py-1.5">
                <div className="flex justify-between"><span>{e.email_type} → {e.recipient_email}</span><span className="text-xs text-muted-foreground">{new Date(e.created_at).toLocaleDateString("fr-FR")}</span></div>
              ...
              <li key={e.id} className="py-1.5">
                <div className="flex justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {e.company_id && <Link to="/admin/companies/$id" params={{ id: e.company_id }} className="mr-2 hover:underline">Fiche →</Link>}
