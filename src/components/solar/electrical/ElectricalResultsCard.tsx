...
export function ElectricalResultsCard({
  companyId,
  modelId,
  expert,
}: {
  ...
  useEffect(() => {
    load({ data: { companyId, modelId } })
      .then((c) => setCtx(c as Ctx))
      .catch(() => setCtx(null));
  ...
  if (!d) {
    return (
      <Card className="p-3 text-xs text-muted-foreground">
        Conception électrique : non réalisée.
      </Card>
    );
  }
  if (ctx.designStale) {
  ...
  return (
    <Card className="space-y-2 p-3 text-sm">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Badge variant={d.status === "valide" ? "default" : "outline"}>
          {DESIGN_STATUS_LABEL[d.status] ?? d.status}
        </Badge>
      </div>
      <dl className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
        <div>
          <dt className="text-muted-foreground">Onduleur</dt>
          <dd>
            {d.inverter_count} × {inv.manufacturer} {inv.model}
          </dd>
        </div>
        <div>
        ...
        <div>
          <dt className="text-muted-foreground">AC / DC</dt>
          <dd>
            {kw(d.summary.ac_power_w)} / {kw(d.summary.dc_power_w)}
          </dd>
        </div>
        <div>
          <dt className="text-muted-foreground">Ratio DC/AC</dt>
          <dd>{d.summary.dc_ac_ratio ?? "Non vérifiable"}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">
            {d.topology === "micro" ? "Micro-onduleurs" : "Strings"}
          </dt>
          <dd>{d.summary.strings}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">MPPT utilisés</dt>
          <dd>{d.summary.mppts.length || "—"}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">Non raccordés</dt>
          <dd>{d.summary.unassigned}</dd>
        </div>
      </dl>
      {d.warnings.length > 0 && (
        <ul className="list-disc pl-4 text-xs text-muted-foreground">
          {d.warnings.slice(0, expert ? undefined : 5).map((w, i) => (
            <li key={i}>
              {w.scope} — {w.message}
            </li>
          ))}
