...
export const listReserveLifts = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rows } = await supabaseAdmin
      .from("reserve_lift_reports")
      .select("id,numero,status,signed_at,pdf_url,pdf_internal_url,pdf_client_url,pdf_generation_status,created_at,client_validated_at,client_validated_email,client_rejected_at,client_signature,validation_mode")
      .eq("pv_id", data.pvId)
