...
export function electricalPdfSections(
  ...
  const rows = [
    ...
    { label: "Type", value: `${TOPOLOGY_LABEL[design.topology] ?? design.topology}${inv.phase ? ` - ${inv.phase === "tri" ? "triphasé" : "monophasé"}` : ""}` },
  ...
  const out: PdfSection[] = [{
    ...
    rows: [
      ...rows,
      { label: "Ratio DC/AC", value: s.dc_ac_ratio == null ? "Non vérifiable" : String(s.dc_ac_ratio) },
      { label: "Statut", value: DESIGN_STATUS_LABEL[design.status] ?? design.status },
      { label: "Températures", value: `Tmin ${design.temp_min_c} °C / Tmax ${design.temp_max_c} °C - ${design.temp_source}` },
      { label: "Fiche onduleur", value: `${inv.provenance} (révision ${inv.revision_id.slice(0, 8)})` },
      { label: "Versions", value: `moteur ${design.engine_version} - implantation v${design.layout_version} - toiture v${design.geometry_version}` },
    ...
    table: {
      ...
      rows: design.groups.map((g) =>
        design.topology === "micro"
          ...
          : [g.label, String(g.inverter_index + 1), g.mppt_index == null ? "-" : String(g.mppt_index + 1), String(g.module_ids.length)],
      ),
    },
  });
  if (s.mppts.length) {
    out.push({
      ...
      table: {
        ...
        rows: s.mppts.map((m) => [
          `${m.inverter_index + 1}/${m.mppt_index + 1}`, String(m.strings),
  ...
  if (design.warnings.length) {
    out.push({
      ...
      text: design.warnings.map((w) => `- ${w.scope} : ${w.message.replace(/≤/g, "<=").replace(/≥/g, ">=")}`).join("\n"),
    });
  }
  return out;
}
