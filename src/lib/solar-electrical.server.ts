...
export async function loadElectricalContext(sb: SB, companyId: string, modelId: string): Promise<ElectricalContext> {
  ...
  const [hash, geo, planes, arrays, modules, invs, revs] = await Promise.all([
    ...
    sb.from("solar_arrays").select("id, roof_plane_id, module_variant_id, module_revision_id, module_snapshot").eq("model_id", modelId).eq("company_id", companyId),
    sb.from("solar_modules_placed").select("id, array_id, roof_plane_id, local_u_m, local_v_m, orientation, enabled").eq("model_id", modelId).eq("company_id", companyId),
    l.from("solar_inverters").select("*").eq("archived", false).or(`company_id.is.null,company_id.eq.${companyId}`),
    l.from("solar_inverter_revisions").select("*").eq("is_current", true).or(`company_id.is.null,company_id.eq.${companyId}`),
  ]);
  if (hash.error || !hash.data) throw new Error("layout_hash_unavailable");

  const planeRows = planes.data ?? [];
  const planeInfo = new Map(planeRows.map((p) => {
    const g = planeGeometryFromRow(p);
    return [p.id, { key: g?.key ?? p.id, name: p.name ?? "Pan" }];
  }));
  const arrayRows = arrays.data ?? [];
  const variantIds = [...new Set(arrayRows.map((a) => a.module_variant_id).filter(Boolean))] as string[];
  const variants = variantIds.length
    ? await sb.from("solar_module_variants").select("*").in("id", variantIds)
    : { data: [] as Record<string, unknown>[] };
  const variantById = new Map((variants.data ?? []).map((v) => [String((v as { id: string }).id), v as Record<string, unknown>]));
  ...
  for (const a of arrayRows) {
    ...
    if (!v) { keyByArray.set(a.id, null); continue; }
    const el = moduleElectricalFromRow(v, a.module_revision_id ?? null, (a.module_snapshot ?? null) as Record<string, unknown> | null);
  ...
  const elecModules: ElecModule[] = (modules.data ?? [])
    ...
    .map((m) => {
      ...
      return {
        id: m.id, plane_key: p.key, plane_name: p.name,
        orientation: m.orientation === "paysage" ? "paysage" : "portrait",
        module_key: m.array_id ? keyByArray.get(m.array_id) ?? null : null,
        u: Number(m.local_u_m), v: Number(m.local_v_m),
      } as ElecModule;
    })
    .sort((a, b) => (a.id < b.id ? -1 : 1));

  const revByInv = new Map<string, Record<string, unknown>>();
  for (const r of (revs.data ?? []) as Record<string, unknown>[]) revByInv.set(String(r.inverter_id), r);
...
export async function loadCurrentDesign(sb: SB, companyId: string, modelId: string) {
  ...
  const [strings, assigns] = await Promise.all([
    ...
    l.from("solar_electrical_assignments").select("string_id, module_id, position").eq("design_id", design.id).order("position"),
  ]);
  ...
  return {
    ...
    inverter_snapshot: design.inverter_snapshot,
    ...
    summary: design.summary as Record<string, unknown>,
    warnings: design.warnings as unknown[],
    created_at: String(design.created_at),
    groups: ((strings.data ?? []) as Record<string, unknown>[]).map((s) => ({
      id: String(s.id),
